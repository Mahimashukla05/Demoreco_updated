...
<body>
    ...
    <nav>
      ...
      <?php if (isset($_SESSION['username'])): ?>
        <?php
          ...
          $initials = strtoupper(substr($username, 0, 2)); // Ensure uppercase
        ?>
  ...
  <script>
    function openModal(type) {
      ...
      // Reset forms and hide previous messages when opening modal
      loginForm.reset();
      signupForm.reset();
    ...
    function showMessage(message, isSuccess = false) {
      ...
      // Change background for success or error messages
      if (isSuccess) {
      ...
      // Remove existing animation if any, then add for restart
      box.style.animation = 'none';
      void box.offsetWidth; // Trigger reflow
      box.style.animation = 'fadeInOut 4s forwards';
      setTimeout(() => box.style.display = 'none', 4000); // Hide after animation
    }
    ...
    document.getElementById("login-form").addEventListener("submit", function (e) {
      ...
      .then(data => {
        if (data.includes("Login successful!")) {
          ...
          closeModal();
          setTimeout(() => location.reload(), 1000); // Reload page after short delay
        } else {
          showMessage(data); // Display error message from PHP
        }
        this.reset();
      })
      .catch(err => {
    ...
    if (loginInputField) { // Check if element exists before adding listener
    ...
    const confirmPasswordHint = document.getElementById("confirm-password-hint"); // New
    ...
    if (signupPassword) {
        signupPassword.addEventListener("input", () => {
            ...
            if (signupConfirmPassword && signupConfirmPassword.value.length > 0) { // Added null check
    ...
    // Full form validation on submit (client-side) for signup
    document.getElementById("signup-form").addEventListener("submit", function (e) {
    ...
    // Full form validation on submit (client-side) for login
    document.getElementById("login-form").addEventListener("submit", function (e) {
        const value = loginInputField.value.trim();
